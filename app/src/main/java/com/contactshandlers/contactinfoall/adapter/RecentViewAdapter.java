...
public class RecentViewAdapter extends RecyclerView.Adapter<RecentViewAdapter.ViewHolder> {
    ...
    private List<RecentViewedContact> recentViewedContacts;

    public RecentViewAdapter(Context context, List<RecentViewedContact> recentViewedContacts) {
        this.context = context;
        this.recentViewedContacts = recentViewedContacts;
    }

    public void setRecentViewedContacts(List<RecentViewedContact> recentViewedContacts) {
        this.recentViewedContacts = recentViewedContacts;
    ...
    public void onBindViewHolder(@NonNull RecentViewAdapter.ViewHolder holder, int position) {
        RecentViewedContact recentViewedContact = recentViewedContacts.get(position);

        String contactId = recentViewedContact.getContactId();
        ...
        holder.binding.tvName.setText(recentViewedContact.getName());

        long timestamp = recentViewedContact.getViewedTimestamp();

        String formattedDate = getFormattedDate(timestamp, context);
        holder.binding.tvContactDate.setText(formattedDate);

        if (position == recentViewedContacts.size() - 1) {
    ...
    public int getItemCount() {
        return recentViewedContacts.size();
    }
