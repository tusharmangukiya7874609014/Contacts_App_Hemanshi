...
import com.contactshandlers.contactinfoall.databinding.ActivityViewHistoryBinding;
...
public class ViewHistoryActivity extends BaseActivity implements View.OnClickListener {
    ...
    private final ViewHistoryActivity activity = ViewHistoryActivity.this;
    ...
    private boolean hasCallLogsPermission = false;
    ...
    private final AtomicBoolean isInitialLoadComplete = new AtomicBoolean(false);
    private final AtomicBoolean isBackgroundLoading = new AtomicBoolean(false);
    ...
    private void setupAds() {
        boolean isAdStart = SharedPreferencesManager.getInstance().getBooleanValue(Constants.VIEW_HISTORY_ACTIVITY_AD_START, false);
        String adType = SharedPreferencesManager.getInstance().getStringValue(Constants.VIEW_HISTORY_ACTIVITY_AD_TYPE, "");
        boolean isAdaptiveBanner = SharedPreferencesManager.getInstance().getBooleanValue(Constants.IS_VIEW_HISTORY_ADAPTIVE_BANNER, false);
        String adaptiveBannerId = SharedPreferencesManager.getInstance().getStringValue(Constants.VIEW_HISTORY_ADAPTIVE_BANNER_ID, "");
        String nativeId = SharedPreferencesManager.getInstance().getStringValue(Constants.VIEW_HISTORY_NATIVE_ID, "");
    ...
    private void init() {
        contactNumber = getIntent().getStringExtra(Constants.CONTACT_NUMBER);
        ...
        binding.rvCallHistory.setLayoutManager(new LinearLayoutManager(activity));
        adapter = new CallHistoryAdapter(activity, new ArrayList<>());
        ...
        loadInitialCallHistory();

        setupContactInfo();
    }

    private void loadInitialCallHistory() {
        backgroundExecutor.execute(() -> {
            try {
                List<CallHistory> initialCallHistory = CallHistoryHelper.getCallLogsForContactWithPagination(
                        activity, contactNumber, 0, INITIAL_LOAD_SIZE);

                if (!activity.isFinishing() && binding != null) {
                    runOnUiThread(() -> {
                        adapter.setCallLogList(initialCallHistory);
                        currentOffset.set(INITIAL_LOAD_SIZE);
    ...
    private void startBackgroundLoading() {
        if (isBackgroundLoading.getAndSet(true)) {
            return;
        }

        backgroundExecutor.execute(() -> {
            try {
                while (!activity.isFinishing() && isInitialLoadComplete.get()) {
                    int offset = currentOffset.get();

                    List<CallHistory> batchData = CallHistoryHelper.getCallLogsForContactWithPagination(
                            activity, contactNumber, offset, BATCH_SIZE);
                    ...
                    runOnUiThread(() -> {
                        if (!activity.isFinishing() && binding != null) {
                            adapter.addCallLogData(batchData);
                        }
                    });
    ...
    private void setupContactInfo() {
        contactName = Utils.getContactNameByNumber(activity, contactNumber);
        if (contactName == null) {
            setupUnknownContact();
        } else {
            setupKnownContact();
    ...
    private void setupUnknownContact() {
        ...
        binding.tvNumber.setText(contactNumber);
        binding.included.llEdit.setVisibility(View.GONE);
        binding.included.btnAdd.setVisibility(View.VISIBLE);
    ...
    private void setupKnownContact() {
        ...
        binding.tvNumber.setText(contactNumber);
        binding.included.llEdit.setVisibility(View.VISIBLE);
        binding.included.btnAdd.setVisibility(View.GONE);

        contactId = Utils.getContactIdByName(activity, contactName);
        isStarred = Utils.isContactStarred(activity, contactId);
        binding.included.ivStar.setImageResource(isStarred ? R.drawable.ic_star_fill : R.drawable.ic_star);

        String type = Utils.getPhoneTypeByNumber(activity, contactNumber);
        phoneList.add(new PhoneItem(contactNumber, type));
    ...
    private void setupContactPhoto() {
        ...
        if (photo != null) {
            ...
            binding.ivProfile.setImageBitmap(Utils.getInitialsBitmap(
                    getColor(R.color.grey_font),
    ...
    private void setupContactEmail() {
    ...
    private void setupFavoriteStatus() {
        isFavourite = Utils.isContactStarred(activity, contactId);
        binding.included.ivStar.setImageResource(isFavourite ? R.drawable.ic_star_fill : R.drawable.ic_star);
    }

    private void setBlockStatus() {
        popup = new PopupMenu(activity, binding.included.ivMore);
    ...
    private void initListener() {
        ...
        binding.included.ivStar.setOnClickListener(this);
        binding.included.btnAdd.setOnClickListener(this);
        binding.included.ivEdit.setOnClickListener(this);
        binding.included.ivMore.setOnClickListener(this);
    }
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.ivBack) {
            ...
            Utils.sendSMS(activity, contactNumber);
            ...
            InterstitialAD.getInstance().showInterstitial(activity, new AdCallback() {
                ...
                public void callbackCall() {
                    startActivity(new Intent(activity, AddEditContactActivity.class)
                            .putExtra(Constants.CONTACT_NUMBER, contactNumber)
                            .putExtra(Constants.CONTACT_NAME, contactName)
                            .putExtra(Constants.TITLE, getString(R.string.edit_contact)));
                }
            });
        } else if (id == R.id.ivStar) {
            boolean newStarStatus = !isStarred;
            if (Utils.toggleContactStarred(activity, contactId, newStarStatus) > 0) {
                isStarred = newStarStatus;
                binding.included.ivStar.setImageResource(isStarred ?
    ...
    private void makePhoneCall() {
        ...
        if (Utils.isDefaultDialer(activity)) {
            Utils.callContact(activity, contactNumber);
        } else {
    ...
    private void showPopupMenu() {
        ...
        popup.setOnMenuItemClickListener(item -> {
            if (item.getItemId() == R.id.action_delete) {
                ...
                return true;
            } else if (item.getItemId() == R.id.action_set_ringtone) {
                requestPermissions();
                return true;
    ...
    private void checkCallLogsPermission() {
        hasCallLogsPermission = ContextCompat.checkSelfPermission(activity, Manifest.permission.READ_CALL_LOG) == PackageManager.PERMISSION_GRANTED &&
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (requestCode == REQUEST_CONTACTS_PERMISSION) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                hasCallLogsPermission = true;
                init();
            } else {
                hasCallLogsPermission = false;
            }
        }
    }
